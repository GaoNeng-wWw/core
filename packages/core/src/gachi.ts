...
export class gachi{
	...
	private configLocation = `${join(this.location, 'config.ts')}`;
	private config:config;
	private spider: Spider;
	private failUrls: urls = [];
	public  pipelines:pipe[] = [];
	private parser: BaseParser;
	private requestTable: Record<string, (option:RequestOption)=>Promise<Record<string, unknown>>> = {
		'http:': http,
		'https:': https
	};
	...
	public async request(this: gachi, option: optionFactory, cb?: (this: Spider, res: unknown, config: customConfig) => unknown, error?: (reason: Error) => void){
		let res: unknown;
		try {
	...
	private async req(urls: urls){
		if (typeof urls !== 'string'){
			for (let i=0;i<urls.length;i++){
				...
				const requestOption = url2Option(url as string[] | urls);
